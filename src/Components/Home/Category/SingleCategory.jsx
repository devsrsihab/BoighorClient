    ...
    >
      <div className="inner-section w-[80%] mx-auto">
        ...
        <div
          ...
          className={`my-16 ${books.length === 0 ? 'grid-cols-1' : 'grid-cols-4 gap-6 '} grid  `}
        >
          {
            books.length === 0 ? <div className="text-center bg-base-400"> <h1 className="py-10 text-2xl text-white text-center ">No books found</h1> </div> :
            books?.map(book =>  
          <BookCard
            key={book._id}
            image={book.image}
